...
## 🌟 核心功能
*   **雙核心 API 備援系統**：自動排序 `WebScraping.AI` > `ScrapingAnt`，具備全球住宅代理，自動繞過所有擋牆。
...
### [2026-04-05]
- **功能改進**：擴充至 6 組付費 API（共 3 組 `WebScraping.AI`、3 組 `ScrapingAnt`），建構高達六道防線的終極自動接力備援機制，最大化抓取承載力。
...
### [2026-04-05] - 雙核心 API 究極版 V17
- **[重要]** 修復兩家 API (WebScraping.AI, ScrapingAnt) 額度查詢欄位不對齊問題。
- **[新增]** 即時顯示兩大 API 的 **「總額度」** 與 **「重置日期時間」** (儀表板全功能)。
